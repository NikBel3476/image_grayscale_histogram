...
bool ImageViewer::loadFile(const QString &fileName)
{
		...
		QImage grayscaleImage = newImage.convertToFormat(QImage::Format_Grayscale8);
		...
		emit imageLoaded(getPixelsData());
...
QList<QColor> ImageViewer::getPixelsData() {
	QList<QColor> pixelsDataList;
	qDebug() << image.height() << image.width();
	for (int i = 0; i < image.width(); i++) {
		for (int j = 0; j < image.height(); j++) {
			pixelsDataList.append(image.pixelColor(i, j));
		}
	}

	return pixelsDataList;
}
